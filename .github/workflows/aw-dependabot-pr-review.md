...
5. **Classify.** Apply the persona's per-surface rubric. Compare ABI-sensitive Python, NumPy, Torch, and CUDA changes against the checked-in runtime image and manifest. Flag changes to those pins, pre-1.0 bumps, major version jumps, and missing upstream advisories.
