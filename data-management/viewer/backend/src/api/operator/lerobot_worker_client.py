...
class LerobotWorkerClient:
    ...
    async def command(
        ...
        return WorkerAcknowledgement(
            ...
            upload_attempted=cleanup.upload_attempted if action == "finish" else False,
            upload_succeeded=cleanup.upload_succeeded if action == "finish" else False,
            upload_error=cleanup.upload_error if action == "finish" else None,
        )
