...
export function TaskCompletenessWidget() {
  ...
  const updateTaskCompleteness = useAnnotationStore((state) => state.updateTaskCompleteness)
  ...
  const subtaskOptions = ['approach', 'grasp', 'lift', 'transport', 'place', 'release', 'retreat']
          ...
          {taskCompleteness?.rating && (
            <span
              className={cn(
                ...
                taskCompleteness.rating === 'unknown' && 'bg-gray-100 text-gray-700',
              )}
            >
              {taskCompleteness.rating}
            </span>
        ...
        {taskCompleteness?.rating === 'failure' && (
          <FormSection label="Failure Reason" htmlFor="failure-reason">
            <Textarea
              ...
              onChange={(e) => updateTaskCompleteness({ failureReason: e.target.value })}
