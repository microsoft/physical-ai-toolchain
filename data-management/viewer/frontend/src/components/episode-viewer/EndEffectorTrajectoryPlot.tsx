...
export function EndEffectorTrajectoryPlot({
    ...
    () =>
      trajectories.map((trajectory) => ({
        ...
        points: trajectory.points
          .slice(0, MAX_END_EFFECTOR_POINTS)
  ...
  useEffect(() => {
    ...
    const currentMarkers = currentMarkersRef.current
    ...
    for (const trajectory of renderedTrajectories) {
      ...
      marker.position.copy(trajectory.points[0])
      currentMarkers.set(trajectory.id, marker)
      scene.add(marker)
    }
  ...
  return (
    <section
      aria-label="End-effector trajectory analysis"
      className={cn('min-w-0 space-y-1', className)}
    >
      <p className="text-muted-foreground text-center text-[10px]">
        Analysis only · not calibration, clearance, or physical accuracy
      </p>
      <div
        aria-label="Trajectory plot frame"
        className={cn('bg-muted/30 relative aspect-video w-full overflow-hidden', frameClassName)}
      >
        <canvas
          ref={canvasRef}
          role="img"
          aria-label="3D end-effector trajectory plot"
        ...
        {!hasPoints && (
          <div className="text-muted-foreground pointer-events-none absolute inset-0 flex items-center justify-center px-4 text-center text-sm">
            No supported recorded trajectory data
          </div>
        )}
