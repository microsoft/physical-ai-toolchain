...
import { AlertTriangle, Check, Cloud, Loader2, RefreshCw, Wifi, WifiOff } from 'lucide-react'
...
export function OfflineIndicator({ className }: OfflineIndicatorProps) {
  const { isOnline, pendingCount, isSyncing, lastSyncResult, sync } = useOfflineAnnotations()
      ...
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm" className={cn('gap-2', className)}>
          {!isOnline ? (
            ...
            <>
              <Loader2
                className={cn('h-4 w-4 animate-spin', getSemanticToneClasses('icon', pendingTone))}
              />
      ...
      <PopoverContent align="end" className="w-72">
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            {hasPending && <Progress value={0} className="h-1.5" />}
          </div>
