    ...
    (set, get) => ({
      ...
      nextEpisode: () => {
        ...
        if (currentIndex < episodes.length - 1) {
          set({ currentIndex: currentIndex + 1, isLoading: true }, false, 'nextEpisode')
        }
      },
      ...
      previousEpisode: () => {
        ...
        if (currentIndex > 0) {
          set({ currentIndex: currentIndex - 1, isLoading: true }, false, 'previousEpisode')
        }
      },
...
export const useCurrentEpisodeIndex = () => useEpisodeStore((state) => state.currentIndex)
