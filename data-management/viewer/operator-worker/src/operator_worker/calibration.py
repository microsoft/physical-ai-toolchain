"""Strict saved SO-101 calibration validation without device access."""
...
class CalibrationError(ValueError):
    """Raised when a saved calibration is unsafe or malformed."""
...
def validate_calibration_file(path: Path) -> dict[str, dict[str, int]]:
    """Validate the backend's exact portable six-joint calibration contract."""
    try:
        if not path.is_file():
    ...
    for motor_id, joint in enumerate(JOINTS, start=1):
        ...
        if not (0 <= values["range_min"] < values["range_max"] <= 4_095):
            raise CalibrationError("Calibration ranges must be ordered within 0 through 4095")
        if not (-2_047 <= values["homing_offset"] <= 2_047):
