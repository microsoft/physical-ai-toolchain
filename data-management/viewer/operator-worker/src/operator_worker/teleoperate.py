"""Deadline-based, bounded teleoperation control."""
...
class TeleoperationLoop:
    """Read fresh actions and stop on duration, request, stale data, or overruns."""
    ...
    def __init__(
        ...
        *,
        ...
        if fps <= 0 or max_duration_s <= 0:
            raise ValueError("fps and max_duration_s must be greater than zero")
        self.leader = leader
    ...
    def run(self, *, max_iterations: int | None = None) -> None:
        ...
        while not self.stop_event.is_set():
            if self.clock() - session_start >= self.max_duration_s:
                return
