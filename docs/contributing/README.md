...
Select a scope from the [commit-message instructions](https://github.com/microsoft/physical-ai-toolchain/blob/main/.github/instructions/commit-message.instructions.md#scopes), such as `infrastructure`, `training`, `scripts`, `docs`, or `build`.
...
Run the applicable tests listed in [Testing Requirements](https://github.com/microsoft/physical-ai-toolchain/blob/main/CONTRIBUTING.md#testing-requirements). Deployment validation does not replace automated tests.
