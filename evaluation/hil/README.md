...
docker run --rm --gpus all --network host \
  ...
  /workspace/isaaclab/isaaclab.sh -p \
    ...
    --config evaluation/hil/config/local-lerobot-ur10e.json \
...
Copy `evaluation/hil/config/local-lerobot-ur10e.json` and replace the USD and prim paths before the run. The ACT policy must be trained from data matching that scene's six-joint, camera, action, and 30 Hz contract.
...
`policy_io.json` must describe the exact `Isaac-Velocity-Rough-Anymal-C-v0` task and policy artifact. Export I/O descriptors from the matching training configuration; do not infer term order from model dimensions.
