 ...
 *
 * System topic on the platform-owned data-lake account (stdl...) that fires
 * BlobCreated events on the `datasets/raw/` ADLS Gen2 path to the conversion
 * subscriber (Function App or Fabric pipeline). Multi-suffix filtering uses
 * advanced_filter.string_ends_with because subject_filter only supports a
 * single suffix value.
 */
...
resource "azurerm_eventgrid_system_topic" "blob" {
  ...
  source_resource_id  = var.data_lake_storage_account.id
...
resource "azurerm_eventgrid_system_topic_event_subscription" "raw_blob_created" {
  ...
  subject_filter {
    subject_begins_with = "/blobServices/default/containers/datasets/blobs/raw/"
  }
