...
| `security/Test-BinaryVersionFreshness.ps1` | Compare exact binary pins with upstream releases and detect inconsistent repository copies    |
...
The `Test-BinaryVersionFreshness.ps1` script is invoked by `check-binary-freshness.yml`. It compares exact binary installation pins against the latest upstream releases, writes details to `freshness-results.json`, and emits the `stale-count`, `attention-count`, and `total-count` workflow outputs.
