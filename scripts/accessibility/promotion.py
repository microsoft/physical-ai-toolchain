...
_HVE_HASHES = {
    "scripts/runtime_a11y/probe-criteria-map.json": "4297ee41d235f2d66697a669d81c1612e5cf21fbb368268fed57d89aa462684f",
    "scripts/runtime_a11y/package-lock.json": "c7b069b44e7f4e3db6008bad8a0d956e88fd5966e70432781935c6cf081960d3",
    "uv.lock": "0930c70adfa8a4e86fff9e326f13e3dd710cde447c3674036aa25898deba665d",
}
...
def _identifier(value: str) -> str:
    _require(isinstance(value, str) and re.fullmatch(r"[1-9][0-9]*", value) is not None,
...
def validate_run(
    run: dict[str, Any], *, repository: str, run_id: str, current_main: str, promotion: bool = False,
) -> None:
    """Bind a completed Actions run to this repository, workflow and current main."""
    _identifier(run_id)
    _sha(current_main)
    expected = {
        "id": int(run_id), "head_sha": current_main, "head_branch": "main",
...
def validate_artifact(
    artifact: dict[str, Any], *, run_id: str, artifact_id: str, source_sha: str, now: datetime,
    ...
    _require(artifact.get("id") == int(artifact_id) and artifact.get("name") == f"{prefix}-{run_id}",
             "Artifact identity does not match the eligible run")
    _require(artifact.get("expired") is False and _time(artifact.get("expires_at")) > now,
             "Artifact has expired or has unknown retention")
    binding = artifact.get("workflow_run") or {}
    _require(binding.get("id") == int(run_id) and binding.get("head_sha") == source_sha
             and binding.get("head_branch") == "main", "Artifact run binding does not match")


def validate_branch_name(name: str) -> None:
    """Accept only an explicit protected branch name, never a revision expression."""
    _require(isinstance(name, str) and bool(name) and not name.startswith(("-", "refs/"))
             ...
             and ".." not in name, "Protected reviewer-evidence branch must be explicitly configured")


def validate_review_ref(
    branch: dict[str, Any], comparison: dict[str, Any], *, branch_name: str, revision: str,
) -> None:
    """Require the candidate to be an ancestor of the protected branch head."""
    validate_branch_name(branch_name)
    _sha(revision)
    _require(branch.get("name") == branch_name and branch.get("protected") is True,
...
def _safe_path(name: str) -> PurePosixPath:
    _require(isinstance(name, str) and bool(name) and "\\" not in name and ":" not in name
             and all(part not in {"", ".", ".."} for part in name.split("/")), "Unsafe evidence path")
...
def validate_review_entry(entry: dict[str, Any]) -> None:
    ...
    permitted = str(path) in {"review-registry.json", "package-manifest.json"} or (
        len(path.parts) == 2 and path.parts[0] == "supplements"
        and re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_-]*\.json", path.name) is not None
    )
    _require(permitted and entry.get("mode") == "100644" and entry.get("type") == "blob",
             "Reviewer evidence must contain only allowlisted, non-executable JSON blobs")


def _inventory(root: Path) -> list[dict[str, Any]]:
    _require(root.is_dir() and not root.is_symlink() and not root.is_junction(),
             "Package root must be an ordinary directory")
    files = []
    for path in sorted(root.rglob("*")):
        ...
        files.append({"path": path.relative_to(root).as_posix(), "sizeBytes": len(data),
...
def verify_review_package(root: Path, source_sha: str) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    _require(all(path.relative_to(root).as_posix() == "supplements" for path in root.rglob("*") if path.is_dir()),
             "Unexpected reviewer evidence directory")
    _require(len(actual) <= _MAX_REVIEW_FILES and sum(item["sizeBytes"] for item in actual) <= _MAX_REVIEW_BYTES,
             "Reviewer package exceeds the bounded data budget")
    for item in actual:
        validate_review_entry({**item, "mode": "100644", "type": "blob"})
        _require(not (root / item["path"]).stat().st_mode & 0o111 if os.name != "nt" else True,
                 "Executable reviewer files are forbidden")
    manifest = read_json(root / "package-manifest.json")
    _require(set(manifest) == {"schemaVersion", "sourceRevision", "files"}
             and manifest["schemaVersion"] == "1.0.0" and manifest["sourceRevision"] == source_sha,
    ...
    for item in expected:
        _require(isinstance(item, dict) and set(item) == {"path", "sha256", "sizeBytes"},
                 "Reviewer manifest entry is invalid")
        validate_review_entry({**item, "mode": "100644", "type": "blob"})
    _require(sorted(expected, key=lambda item: item["path"])
...
def validate_anchors(
    prior: dict[str, Any], registry: dict[str, Any], prior_digest: str, registry_digest: str,
) -> None:
    ...
    for document, field, expected, domain in (
        ...
        computed = evidence_gate._hve_canonical_digest(
            {key: value for key, value in document.items() if key != field}, domain,
        )
...
def evaluation_context(original: dict[str, Any], *, now: datetime) -> dict[str, Any]:
    ...
    _require(now.tzinfo is not None and _time(original.get("composedAt")) <= now,
...
def require_release(bundle: dict[str, Any]) -> None:
    ...
    _require((bundle.get("scopeCompleteness") or {}).get("releaseEvidence") == "complete",
...
class GitHub:
    ...
    def __init__(self, repository: str) -> None:
        _require(re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9-]*/[A-Za-z0-9_.-]+", repository) is not None
    ...
    def get(self, route: str) -> dict[str, Any]:
        ...
        result = json.loads(_command(["gh", "api", "--method", "GET", f"repos/{self.repository}/{route}"]),
...
def _read_git_blob(api: GitHub, entry: dict[str, Any]) -> bytes:
    ...
    _require(blob.get("encoding") == "base64" and blob.get("sha") == entry["sha"],
             "Reviewer blob identity is invalid")
    data = base64.b64decode("".join(blob["content"].split()), validate=True)
    _require(len(data) == entry["size"] and len(data) <= _MAX_REVIEW_BYTES,
             "Reviewer blob size differs from its tree")
...
def verify_current_registry(
    api: GitHub, *, branch_name: str, reviewer_revision: str, source_sha: str,
    promoted_digest: str, expected_digest: str,
) -> None:
    ...
    _require(isinstance(entry.get("size"), int) and 0 <= entry["size"] <= _MAX_REVIEW_BYTES,
    ...
    digest = evidence_gate._hve_canonical_digest(
        {key: value for key, value in registry.items() if key != "digest"}, "hve-a11y:review-registry:v1",
    )
    _require(registry.get("digest") == digest == expected_digest,
...
def acquire_review(api: GitHub, *, revision: str, branch_name: str, source_sha: str, output: Path) -> None:
    ...
    _require(0 < len(selected) <= _MAX_REVIEW_FILES
             and sum(item["size"] for item in selected) <= _MAX_REVIEW_BYTES,
...
def verify_harness(harness_root: Path) -> None:
    ...
    _require(_command(["git", "-C", str(checkout), "rev-parse", "HEAD"]) == _HVE_REF,
             "HVE revision differs from the reviewed collection pin")
    _require(_command(["git", "-C", str(checkout), "rev-parse", f"HEAD:{_HVE_PATH}"]) == _HVE_TREE,
    ...
    _require(not _command(["git", "-C", str(checkout), "diff", "HEAD", "--", _HVE_PATH]),
             "HVE tracked runtime was modified")
    for relative, expected in _HVE_HASHES.items():
        _require(hashlib.sha256((root / relative).read_bytes()).hexdigest() == expected,
...
def acquire_harness(checkout: Path) -> None:
    ...
    _command(["git", "-C", str(checkout), "checkout", "--quiet", "--detach", _HVE_REF])
...
def _validate_collection(root: Path, harness_root: Path, source_sha: str) -> dict[str, Any]:
    ...
    _require(read_json(metadata / "inputs/evidence-scope.json").get("cadenceClass") == "release",
             "Collection must cover release scope")
    summary = evidence_gate.validate_composed_docusaurus_bundle(
        metadata / "evidence-bundle.json", root, harness_root=harness_root, required_completeness="automated",
    )
...
def _compose(
    collection: Path, review: Path, output: Path, *, harness_root: Path, prior_digest: str,
    registry_digest: str, now: datetime,
) -> Path:
    ...
    _require(evidence_gate._hve_canonicalize(original) == evidence_gate._hve_canonicalize(prior["runManifest"]),
             "Collection run context differs from the original bundle")
    for supplement in supplements:
        _require(all(supplement.get(key) == original.get(key)
    ...
    arguments = [
        sys.executable, str(harness_root / "scripts/runtime_a11y/__main__.py"), "compose-evidence",
        "--asset-catalog", str(inputs / "asset-journeys.json"),
        "--requirement-catalog", str(inputs / "requirement-methods.json"),
        "--scope", str(inputs / "evidence-scope.json"), "--run-context", str(context_path),
        "--source", str(inputs / "evidence-source.json"), "--source", str(inputs / "evidence-playwright.json"),
        "--state-proofs", str(inputs / "state-proofs.json"), "--artifact-root", str(collection),
        "--prior-bundle", str(prior_path), "--prior-bundle-digest", prior_digest,
        "--review-registry", str(review / "review-registry.json"),
        "--expected-review-registry-digest", registry_digest,
        "--out", str(bundle_path), "--require-completeness", "release",
    ]
    ...
    _require(evidence_gate._hve_canonicalize(bundle["runManifest"]) == evidence_gate._hve_canonicalize(evaluated),
             "Recomposition changed collection identity")
    summary = evidence_gate.validate_composed_docusaurus_bundle(
        bundle_path, collection, harness_root=harness_root, required_completeness="release",
    )
...
def promote(
    collection: Path, review: Path, output: Path, *, harness_root: Path, source_sha: str,
    collection_run_id: str, collection_artifact_id: str, promotion_run_id: str, reviewer_revision: str,
    prior_digest: str, registry_digest: str, now: datetime,
) -> None:
    ...
    bundle_path = _compose(
        (output / "collection").resolve(), (output / "review").resolve(), (output / "promotion").resolve(),
        harness_root=harness_root.resolve(), prior_digest=prior_digest, registry_digest=registry_digest, now=now,
    )
    promoted = read_json(bundle_path)
    validation = {
        "schemaVersion": "1.0.0", "sourceRevision": source_sha,
        "buildDigest": prior["runManifest"]["buildDigest"], "bundleDigest": promoted["bundleDigest"],
        "priorBundleDigest": prior_digest, "reviewRegistryDigest": registry_digest,
        "reviewerRevision": reviewer_revision, "collectionRunId": collection_run_id,
        "collectionArtifactId": collection_artifact_id, "promotionRunId": promotion_run_id,
        "hveRevision": _HVE_REF, "hveTree": _HVE_TREE, "releaseEvidence": "complete", "attestation": False,
...
def validate_promotion_manifest(
    validation: dict[str, Any], prior: dict[str, Any], bundle: dict[str, Any], *,
    source_sha: str, promotion_run_id: str,
) -> None:
    """Bind deployment metadata to the original and promoted evidence identities."""
    fields = {
        "schemaVersion", "sourceRevision", "buildDigest", "bundleDigest", "priorBundleDigest",
        "reviewRegistryDigest", "reviewerRevision", "collectionRunId", "collectionArtifactId",
        "promotionRunId", "hveRevision", "hveTree", "releaseEvidence", "attestation", "evaluatedAt",
    }
    _require(set(validation) == fields and validation["schemaVersion"] == "1.0.0",
    ...
    _require(validation["sourceRevision"] == _sha(source_sha) == prior["runManifest"]["sourceRevision"]
             and validation["promotionRunId"] == _identifier(promotion_run_id)
             and validation["hveRevision"] == _HVE_REF and validation["hveTree"] == _HVE_TREE
             and validation["releaseEvidence"] == "complete" and validation["attestation"] is False,
             "Promoted validation manifest does not identify this release")
    require_release(bundle)
    _require(bundle.get("bundleDigest") == validation["bundleDigest"]
...
def verify_promoted(
    root: Path, *, harness_root: Path, source_sha: str, promotion_run_id: str, scratch: Path,
    now: datetime,
) -> dict[str, Any]:
    ...
    _require(set(manifest) == {"schemaVersion", "files"} and manifest["schemaVersion"] == "1.0.0",
    ...
    summary = evidence_gate.validate_composed_docusaurus_bundle(
        root / "promotion/evidence-bundle.json", root / "collection",
        harness_root=harness_root, required_completeness="release",
    )
    _require(summary.get("verdict") == "PASS", "Promoted release verdict is incomplete")
    build_root = root / "collection/docs/docusaurus/build"
    digest = evidence_gate._digest_paths([path for path in build_root.rglob("*") if path.is_file()],
                                        root / "collection")
    _require(digest == validation.get("buildDigest") == prior["runManifest"]["buildDigest"],
             "Retained documentation build digest does not match")
    _require(not scratch.exists() and not scratch.resolve().is_relative_to(root.resolve()),
             "Expiry verification requires a fresh directory outside the package")
    try:
        current_bundle_path = _compose(
            (root / "collection").resolve(), (root / "review").resolve(), scratch.resolve(),
            harness_root=harness_root.resolve(), prior_digest=validation["priorBundleDigest"],
            registry_digest=validation["reviewRegistryDigest"], now=now,
        )
...
def _check_run(api: GitHub, run_id: str, *, promotion: bool) -> str:
    ...
    validate_run(api.get(f"actions/runs/{run_id}"), repository=api.repository,
                 run_id=run_id, current_main=source_sha, promotion=promotion)
    return source_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("operation", choices=[
        "check-collection", "acquire-review", "acquire-harness", "promote", "check-promotion",
        "verify-promoted", "check-current",
    ])
    ...
    try:
        ...
        if args.operation == "verify-promoted":
            validation = verify_promoted(
                args.output, harness_root=args.harness_root, source_sha=args.source_sha,
                promotion_run_id=args.promotion_run_id, scratch=args.scratch, now=now,
            )
            if args.refresh_review_registry:
                verify_current_registry(
                    GitHub(args.repository), branch_name=args.reviewer_branch,
                    reviewer_revision=validation["reviewerRevision"], source_sha=validation["sourceRevision"],
                    promoted_digest=validation["reviewRegistryDigest"], expected_digest=args.registry_digest,
                )
            _github_output(source_sha=validation["sourceRevision"], build_digest=validation["buildDigest"],
        ...
        if args.operation == "check-current":
            ...
            validate_artifact(api.get(f"actions/artifacts/{args.artifact_id}"), run_id=args.run_id,
                              artifact_id=args.artifact_id, source_sha=source, now=now)
            _github_output(source_sha=source)
        elif args.operation == "acquire-review":
            _sha(args.registry_digest, 64)
            acquire_review(api, revision=args.reviewer_revision, branch_name=args.reviewer_branch,
            ...
            validate_artifact(artifact, run_id=args.run_id,
                              artifact_id=artifact_id, source_sha=source, now=now, promotion=True)
            ...
            validate_artifact(api.get(f"actions/artifacts/{args.artifact_id}"), run_id=args.run_id,
                              artifact_id=args.artifact_id, source_sha=source, now=now)
            promote(
                args.collection, args.review, args.output, harness_root=args.harness_root, source_sha=source,
                collection_run_id=args.run_id, collection_artifact_id=args.artifact_id,
                promotion_run_id=args.promotion_run_id, reviewer_revision=args.reviewer_revision,
                prior_digest=args.prior_bundle_digest, registry_digest=args.registry_digest, now=now,
            )
