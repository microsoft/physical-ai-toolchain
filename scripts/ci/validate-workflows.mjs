...
function excludesRootEvents(value, rootEvents, declaredEvents) {
  if (typeof value !== 'string' || rootEvents.length === 0) return false;
  const trimmed = value.trim();
  const condition = trimmed.startsWith('${{') && trimmed.endsWith('}}') ? trimmed.slice(3, -2) : trimmed;
  const selectedEvents = condition.split('||').map(term =>
    /^\s*github\.event_name\s*==\s*'([a-z][a-z0-9_]*)'\s*$/i.exec(term)?.[1].toLowerCase());
  return selectedEvents.every(event => event && declaredEvents.some(declared => declared.toLowerCase() === event))
...
export function validateWorkflows(graph, contract = loadContract()) {
  ...
  const checkOwnedDescendants = (path, root, stack = []) => {
    ...
    for (const [id, job] of Object.entries(graph[path]?.jobs ?? {})) {
      ...
      if (targets.has(name)) {
        const key = `${path}:${id}`;
        const secondary = secondaryOwnership.get(key) === name;
        const eventExcluded = root !== path && secondary
          && excludesRootEvents(job.if, executableEvents(graph[root]), contract.secondaryCallers[path].events);
        const authorized = eventExcluded || (root === path && (ownership.get(key)?.target === name || secondary));
