...
const hoisted = vi.hoisted(() => {
  ...
  const recordDiagnosticEvent = vi.fn((channel: string, type: string, data?: Record<string, unknown>) => {
  ...
  return {
    ...
    computeSyncAction: vi.fn<() => { kind: string; playbackRate?: number }>(() => ({ kind: 'pause' })),
...
vi.mock('@/components/object-detection', () => ({ DetectionPanel: () => <div>Detection Panel</div> }))
...
vi.mock('@/components/viewer-display', () => ({ ViewerDisplayControls: () => <div>Viewer Display Controls</div> }))
...
vi.mock('@/lib/playback-diagnostics', () => ({
  ...
  getEnabledDiagnosticsChannels: () => (hoisted.diagnosticsState.enabled ? hoisted.diagnosticsState.channels : []),
...
vi.mock('@/lib/playback-utils', () => ({
  ...
  resolvePlaybackRange: (totalFrames: number, range: [number, number] | null) => range ?? [0, totalFrames - 1],
  ...
  useLabelStore: (selector: (state: unknown) => unknown) =>
    selector({
      ...
      commitEpisodeLabels: (episodeIndex: number, labels?: string[]) => {
        ...
        hoisted.state.savedEpisodeLabels = { ...hoisted.state.savedEpisodeLabels, [episodeIndex]: nextLabels }
      },
    }),
}))
...
export function setupAnnotationWorkspaceTestCase() {
  vi.useFakeTimers()
  mediaSpies.play = vi.spyOn(HTMLMediaElement.prototype, 'play').mockImplementation(() => Promise.resolve())
  mediaSpies.pause = vi.spyOn(HTMLMediaElement.prototype, 'pause').mockImplementation(() => undefined)
