...
describe('useAnnotationWorkspaceMediaController', () => {
  ...
  it('derives the primary camera video and frame URLs for the current episode frame', () => {
    const { result } = renderHook(() => useAnnotationWorkspaceMediaController({
    ...
    expect(result.current.frameImageUrl).toBe('/api/datasets/dataset-1/episodes/3/frames/4?camera=wrist')
  })
