...
export function useAnnotationWorkspaceDiagnostics({
  ...
  const diagnosticsStateSummary = useMemo(() => buildDiagnosticsStateSummary({
    ...
    selectedRange,
    selectedSubtaskId,
  }), [activeTab, currentDatasetId, currentEpisodeIndex, currentFrame, diagnosticsChannels, isPlaying, selectedRange, selectedSubtaskId, totalFrames])
  ...
  const handleDownloadDiagnostics = useCallback(() => {
    ...
    link.href = url
    link.download = selectedDiagnosticsChannel === 'all'
