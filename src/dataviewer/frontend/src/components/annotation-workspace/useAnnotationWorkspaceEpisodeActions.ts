...
export function useAnnotationWorkspaceEpisodeActions({
  ...
  useEffect(() => {
    ...
    lastLabelSignatureRef.current = labelSignature;
  }, [currentEpisodeIndex, currentEpisodeLabels, diagnosticsEnabled, hasLabelChanges, labelSignature, onRecordEvent]);

  useEffect(() => {
    const nextContext = currentDatasetId !== null && currentEpisodeIndex !== null
  ...
  }, [availableLabels, currentEpisodeIndex, hasLabelChanges, onResetEdits, onSetEpisodeLabels, savedLabelsForCurrentEpisode]);
  ...
  const handleSaveAndNextEpisode = useCallback(async () => {
    ...
    onAdvanceToNextEpisode();
  }, [announceSave, canGoNextEpisode, currentDatasetId, currentEpisodeIndex, currentEpisodeLabels, hasEdits, hasLabelChanges, hasPendingEpisodeChanges, onAdvanceToNextEpisode, onRecordEvent, onSaveEpisodeDraft, onSaveEpisodeLabels]);
