...
export function TransformControls({
  ...
  const [width, setWidth] = useState<string>(
    currentResize?.width?.toString() ?? ''
  );
  const [height, setHeight] = useState<string>(
    currentResize?.height?.toString() ?? ''
  );
  ...
  const handleResetResize = useCallback(() => {
    ...
    if (cameraName) {
      ...
      setGlobalTransform(
        globalTransform?.crop ? { crop: globalTransform.crop } : null
      );
    }
    ...
    <div className={cn('flex flex-col gap-4', className)}>
      ...
      {globalTransform && (
        <div className="text-xs text-muted-foreground p-2 bg-muted rounded">
          <div className="font-medium mb-1">Current Transform:</div>
          {globalTransform.crop && (
            <div>
              Crop: {globalTransform.crop.width}×{globalTransform.crop.height}{' '}
              at ({globalTransform.crop.x}, {globalTransform.crop.y})
            </div>
          )}
          {globalTransform.resize && (
            <div>
              Resize: {globalTransform.resize.width}×
              {globalTransform.resize.height}
            </div>
          )}
        </div>
      )}
