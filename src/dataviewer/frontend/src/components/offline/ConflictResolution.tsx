    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={cn('max-w-2xl', className)}>
        <DialogHeader>
          ...
          <DialogDescription>
            The annotation for episode <strong>{episodeId}</strong> was modified
            both locally and on the server. Choose which version to keep.
          </DialogDescription>
        </DialogHeader>

        <RadioGroup value={choice} onValueChange={(v: string) => setChoice(v as 'local' | 'server')}>
