...
export const annotationKeys = {
  ...
  summary: (datasetId: string) =>
    [...annotationKeys.all, 'summary', datasetId] as const,
...
export function useEpisodeAnnotations(annotatorId: string) {
  ...
  const initializeAnnotation = useAnnotationStore(
    (state) => state.initializeAnnotation
  );
  ...
  useEffect(() => {
    if (query.data) {
      ...
      const userAnnotation = query.data.annotations.find(
...
export function useSaveCurrentAnnotation() {
  const currentDataset = useDatasetStore((state) => state.currentDataset);
  const currentIndex = useEpisodeStore((state) => state.currentIndex);
  const currentAnnotation = useAnnotationStore(
...
export function useDeleteAnnotation() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      // Invalidate queries
      queryClient.invalidateQueries({
        queryKey: annotationKeys.detail(
          variables.datasetId,
...
export function useAutoAnalysis() {
  const updateTrajectoryQuality = useAnnotationStore(
    (state) => state.updateTrajectoryQuality
  );
