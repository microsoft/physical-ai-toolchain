...
function computeVectorPathLength(
  ...
  for (let frameIndex = 1; frameIndex < trajectoryData.length; frameIndex += 1) {
    ...
    for (const index of indices) {
      const delta = (trajectoryData[frameIndex].jointPositions[index] ?? 0)
...
function computeQuaternionTravel(
  trajectoryData: TrajectoryPoint[],
  indices: number[],
): number {
  let total = 0
...
export function rankJointGroupsBySignificance(
  ...
  return rawGroups
    .map((group) => {
      ...
      const score = categoryMax >= MIN_CATEGORY_ACTIVITY[group.kind]
...
export function getAutoSelectedJointsForEpisode(
  ...
  return Array.from(new Set(selected.flatMap((group) => group.indices))).sort((left, right) => left - right)
}
