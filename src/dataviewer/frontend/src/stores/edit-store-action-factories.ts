...
export function createEditStoreFrameActions<State extends FrameState>(updateState: UpdateState<State>) {
  return {
    ...
    clearRemovedFrames: () => {
      updateState('clearRemovedFrames', () => ({ removedFrames: new Set<number>() } as Partial<State>))
    },
    ...
    setTrajectoryAdjustment: (frameIndex: number, adjustment: Omit<{ frameIndex: number }, 'frameIndex'>) => {
    ...
    clearTrajectoryAdjustments: () => {
      updateState('clearTrajectoryAdjustments', () => ({ trajectoryAdjustments: new Map() } as Partial<State>))
    },
  }
}
...
export function createEditStoreSubtaskActions<State extends SubtaskState>(
  ...
  return {
    ...
    removeSubtask: (id: string) => {
      updateState(
        'removeSubtask',
        (state) => ({ subtasks: state.subtasks.filter((segment) => segment.id !== id) } as Partial<State>),
        { validationErrors: (nextState) => validateSegments(nextState.subtasks) },
      )
    },
...
export function createEditStoreTransformActions<State extends { globalTransform: unknown; cameraTransforms: Record<string, unknown> }>(updateState: UpdateState<State>) {
  return {
    ...
    clearTransforms: () => {
      updateState('clearTransforms', () => ({ globalTransform: null, cameraTransforms: {} } as Partial<State>))
    },
  }
}
