...
export function buildOriginalEditState(state: Pick<EditStateSnapshot, 'globalTransform' | 'cameraTransforms' | 'removedFrames' | 'insertedFrames' | 'subtasks' | 'trajectoryAdjustments'>): EditOriginalStateSnapshot {
  return {
...
export function buildEditOperations(state: EditStateSnapshot): EpisodeEditOperations | null {
  ...
  return {
    ...
    cameraTransforms: Object.keys(state.cameraTransforms).length > 0 ? state.cameraTransforms : undefined,
    removedFrames: state.removedFrames.size > 0 ? Array.from(state.removedFrames).sort((a, b) => a - b) : undefined,
    insertedFrames: state.insertedFrames.size > 0
      ? Array.from(state.insertedFrames.values()).sort((a, b) => a.afterFrameIndex - b.afterFrameIndex)
      : undefined,
    subtasks: state.subtasks.length > 0 ? state.subtasks : undefined,
    trajectoryAdjustments: state.trajectoryAdjustments.size > 0 ? Array.from(state.trajectoryAdjustments.values()) : undefined,
  }
}
...
export function computeDirty(state: EditStateWithDerived): boolean {
  ...
  for (const [afterIdx, insertion] of state.insertedFrames) {
    ...
    if (!originalInsertion || insertion.interpolationFactor !== originalInsertion.interpolationFactor) {
      return true
    }
  }
