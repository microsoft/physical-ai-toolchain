...
class TestRawBagValidation:
    ...
    def test_valid_raw_mobile_manipulator(self):
        assert validate_blob_path(
            "raw/mobile-manipulator-03/2026-03-01/navigation-001.mcap", "raw"
        )

    def test_invalid_raw_uppercase_device(self):
        assert not validate_blob_path(
            "raw/Robot-01/2026-03-05/episode-001.mcap", "raw"
        )
    ...
    def test_invalid_raw_wrong_date_format(self):
        assert not validate_blob_path(
            "raw/robot-01/03-05-2026/episode-001.mcap", "raw"
        )
...
class TestConvertedDatasetValidation:
    ...
    def test_valid_converted_meta_stats(self):
        assert validate_blob_path(
            "converted/pick-place-v1/meta/stats.json", "converted"
        )

    def test_valid_converted_data_parquet(self):
        assert validate_blob_path(
            "converted/pick-place-v1/data/chunk-000/episode_000000.parquet", "converted"
        )
    ...
    def test_invalid_converted_uppercase(self):
        assert not validate_blob_path(
            "converted/Pick-Place-v1/meta/info.json", "converted"
        )

    def test_invalid_converted_wrong_subfolder(self):
        assert not validate_blob_path(
            "converted/pick-place-v1/invalid/info.json", "converted"
        )
...
class TestReportsValidation:
    ...
    def test_valid_reports_json(self):
        assert validate_blob_path(
            "reports/pick-place-v1/2026-03-05/eval_results.json", "reports"
        )

    def test_valid_reports_npz(self):
        assert validate_blob_path(
            "reports/pick-place-v1/2026-03-05/ep000_predictions.npz", "reports"
        )

    def test_valid_reports_mp4(self):
        assert validate_blob_path(
            "reports/pick-place-v1/2026-03-05/inference_video.mp4", "reports"
        )
    ...
    def test_invalid_reports_wrong_date_format(self):
        assert not validate_blob_path(
            "reports/pick-place-v1/03-05-2026/eval_results.json", "reports"
        )

    def test_invalid_reports_missing_date(self):
        assert not validate_blob_path(
            "reports/pick-place-v1/eval_results.json", "reports"
        )
...
class TestCheckpointsValidation:
    ...
    def test_valid_checkpoints_with_step(self):
        assert validate_blob_path(
            "checkpoints/act-policy/20260305_143022_step_1000.pt", "checkpoints"
        )

    def test_valid_checkpoints_no_step(self):
        assert validate_blob_path(
            "checkpoints/velocity-anymal/20260301_120000.pt", "checkpoints"
        )

    def test_valid_checkpoints_onnx(self):
        assert validate_blob_path(
            "checkpoints/diffusion-policy/20260304_091500.onnx", "checkpoints"
        )

    def test_valid_checkpoints_jit(self):
        assert validate_blob_path(
            "checkpoints/rsl-rl/20260305_100000.jit", "checkpoints"
        )
    ...
    def test_invalid_checkpoints_wrong_timestamp_format(self):
        assert not validate_blob_path(
            "checkpoints/act-policy/2026-03-05_14-30-22_step_1000.pt", "checkpoints"
        )

    def test_invalid_checkpoints_wrong_extension(self):
        assert not validate_blob_path(
            "checkpoints/act-policy/20260305_143022_step_1000.pth", "checkpoints"
        )
