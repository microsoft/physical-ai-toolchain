...
def test_osmo_vla_finetune_e2e(
    ...
    register_cleanup(request, workflow.handle, "stop_task_log_stream", log_stream.stop)
    ...
    provenance = parse_provenance_marker(
        fetch_workflow_task_logs(workflow, repo_root, _VLA_TASK_NAME),
        "VLA_RUNTIME_PROVENANCE=",
    )
