...
def _validate_output_urls(output_urls: list[str]) -> list[str]:
    ...
    for raw_url in output_urls:
        ...
        if parsed.scheme != "azure" or not parsed.netloc or parsed.query or parsed.fragment:
            raise ValueError("OSMO output URLs must use azure:// without query strings or fragments")
        if not _STORAGE_ACCOUNT_PATTERN.fullmatch(parsed.netloc):
            raise ValueError("OSMO output URL contains an invalid storage account name")

        path_parts = parsed.path.removeprefix("/").split("/")
        container = path_parts[0] if path_parts else ""
        blob_path_parts = path_parts[1:]
        has_trailing_slash = bool(blob_path_parts) and blob_path_parts[-1] == ""
        if has_trailing_slash:
            blob_path_parts = blob_path_parts[:-1]
        if not _CONTAINER_PATTERN.fullmatch(container):
            raise ValueError("OSMO output URL contains an invalid container name")
        if any(part in {"", ".", ".."} or "\\" in part for part in blob_path_parts):
            raise ValueError("OSMO output URL contains a non-canonical blob path")

        normalized = f"azure://{parsed.netloc}/{container}"
        if blob_path_parts:
            normalized += f"/{'/'.join(blob_path_parts)}"
        if has_trailing_slash:
            normalized += "/"
...
def _fetch_blob_json(azure_url: str, client_id: str = "") -> dict[str, Any] | None:
    ...
    try:
        ...
        _LOGGER.debug("Blob JSON fetch failed for account=%s container=%s (non-fatal): %s", account, container, exc)
        return None
...
def _register_aml_data_assets(
    ...
    asset_prefix = os.environ.get("AML_OUTPUT_ASSET_PREFIX", workflow_id).strip()
    ...
    for idx, url in enumerate(output_urls):
        asset_name = f"osmo-{asset_prefix}-output-{idx}"
        aml_url = _to_aml_url(url)
        data_asset = Data(
            name=asset_name,
            path=aml_url,
            type=AssetTypes.URI_FOLDER,
            description=f"OSMO workflow {workflow_id} output {idx}",
        )
        created = client.data.create_or_update(data_asset)
        _LOGGER.info(
            "Registered AML data asset: %s v%s",
            created.name,
            created.version,
        )
...
def main() -> None:
    ...
    if args.metrics_only:
        output_urls = _validate_output_urls([u.strip() for u in args.output_urls.split(",") if u.strip()])
    ...
    output_urls = _validate_output_urls(output_urls)
