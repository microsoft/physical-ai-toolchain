...
output_url="${output_url:-azure://${storage_account}/${output_container}/proxy-smoke-test/}"
[[ "$output_url" == azure://*/*/* ]] || fatal "Output URL must use azure://<account>/<container>/<path>"
output_location="${output_url#azure://}"
requested_output_account="${output_location%%/*}"
output_remainder="${output_location#*/}"
requested_output_container="${output_remainder%%/*}"
[[ "$requested_output_account" == "$storage_account" ]] || fatal "Output URL must use deployment storage account"
[[ "$requested_output_container" == "$output_container" ]] || fatal "Output URL must use container: $output_container"
...
az_args=(
  ...
  --set "environment_variables.OSMO_OUTPUT_STORAGE_ACCOUNT=${storage_account}"
  --set "environment_variables.OSMO_OUTPUT_CONTAINER=${output_container}"
...
if [[ -n "$job_name" ]]; then
  az_args+=(--name "$job_name")
  az_args+=(--set "environment_variables.AML_OUTPUT_ASSET_PREFIX=${job_name}")
fi
